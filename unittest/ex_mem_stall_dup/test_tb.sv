// Unit test: ex_mem_stall_dup — reproduce EX/MEM stall duplicate commit
// Key: cacop_done is delayed by 1 cycle, causing EX/MEM stall that
// makes the preceding instruction (or) capture twice by MEM/WB
`include "common.sv"

module test_ex_mem_stall_dup (
    input  logic clk,
    input  logic reset
);
    import la32_common::*;
    ...
    // CACOP: 1-cycle delay (registered), exposing EX/MEM stall bug
    logic cacop_done_r;
    always_ff @(posedge clk) begin
