...
module test_bne_load_use_bpflush (
    input  logic clk,
    input  logic reset
);
    import la32_common::*;
    ...
    assign dresp.data      = 32'h00000094;
    ...
    always_ff @(posedge clk) begin
        ...
        if (!reset && debug_cycle < 40) begin
            $display("[C%0d] pc=%08x ireq=%08x iok=%0d", debug_cycle, u_core.pc, ireq.addr, iresp.data_ok);
            $display("  IF v%0d %08x ID v%0d %08x EM v%0d %08x MW v%0d %08x | st=%0d fl=%0d exj=%0d bpj=%0d luh=%0d",
                u_core.if_id_out.ctrl.valid, u_core.if_id_out.data.pc,
                u_core.id_ex_out.ctrl.valid, u_core.id_ex_out.data.pc,
                u_core.ex_mem_out.ctrl.valid, u_core.ex_mem_out.data.pc,
                u_core.mem_wb_out.ctrl.valid, u_core.mem_wb_out.data.pc,
                u_core.ex_mem_stall, u_core.if_id_flush || u_core.id_ex_flush,
                u_core.ex_jump_flush, u_core.bp_do_jump, u_core.load_use_hazard);
        end
    end
