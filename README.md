...
>
> **⚠ 当前 100MHz 零 IPC 进展（impl WNS -0.489 → -0.304）**：两个零 IPC 结构改动已提交——① 接受侧覆盖搜索改读寄存的预览地址（`0533137`，impl -0.455，搜索的 LSU/arbiter 地址 mux 链出路径）；② 数据总线三态选择提前一拍预计算（`c3f51fb`，impl -0.304，借鉴 [Wubian111/Wubian_la32r_cpu](https://github.com/Wubian111/Wubian_la32r_cpu) MemoryIO.v 的写选择寄存化，已加入致谢）。**SRAM T 族从 impl 违例集彻底消失**（IOB 偏斜/OBUFT 地板被"选择寄存化"绕开）。新顶部 = **pc_reg 族（-0.304，44 条）——0-cycle icache 响应链**（`pc_reg[4]（fo=84）→ tag LUTRAM → icache FSM → fetch FSM → FQ 吸收 → fq_pc[0]`，13 级、路由主导）——即第四轮想砍而未砍断的族。两次改动的门禁 IPC 均与基线完全一致（零 IPC 实测确认）。
>
